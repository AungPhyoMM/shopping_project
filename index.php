...
if (empty($_POST['search']) && empty($_COOKIE['search'])) {
	if (!empty($_GET['categories_id'])) {
		...
		$stmt = $pdo->prepare("SELECT * FROM products WHERE categories_id=$categoryId AND quantity > 0 ORDER BY id DESC");
		$stmt->execute();
		$rawResult = $stmt->fetchAll();

		$total_pages = ceil(count($rawResult) / $numOfrecs);

		$stmt = $pdo->prepare("SELECT * FROM products WHERE categories_id=$categoryId AND quantity > 0 ORDER BY id DESC LIMIT $offset,$numOfrecs");
		$stmt->execute();
				...
				<ul class="main-categories">
					<li class="main-nav-list">
						...
						<?php foreach ($catResult as $key => $value) { ?>
							<a href="index.php?categories_id=<?php echo $value['id'] ?>"><span class="lnr lnr-arrow-right"></span><?php echo escape($value['name']) ?></a>
						<?php  } ?>
					</li>
				</ul>
			</div>
